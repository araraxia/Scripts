#!/usr/bin/env python3
"""
Service Account Email Sender

This is an alternative to AutomatedEmails.py that uses service account authentication
with optional user impersonation instead of OAuth2 user credentials.

This approach is better for:
- Automated systems that don't require user interaction
- Sending emails from shared accounts (e.g., no-reply@example.com)
...
Example Usage:
    # Basic usage without impersonation
    email_sender = ServiceAccountEmailSender(
    ...
    email_sender.send_email(
        ...
        subject='Test Email',
        body='This is a test email sent via service account',
        file_attachment_paths=['path/to/attachment.pdf']
    )
"""
...
class Thread:
    ...
    Example:
        ...
        ... )
        >>> thread = manager.get_thread_by_id('thread123')
        ...
        >>> thread.modify_labels(add_label_ids=['STARRED'], remove_label_ids=['UNREAD'])
        >>>
    ...
    def __init__(self, raw_thread: dict = None, thread_id: str = None, manager=None):
        ...
        Example:
            >>> # From raw thread data
            >>> thread = Thread(raw_thread={'id': 'abc', 'messages': [...]})
            >>>
            >>> # Lazy initialization
            >>> thread = Thread(thread_id='abc123', manager=email_manager)
        """
    ...
    def remove_message(self, message):
        ...
        Example:
            >>> # Remove a message (thread continues)
            >>> was_deleted = thread.remove_message(some_message)
            >>> print(was_deleted)  # False - message deleted, thread continues
            >>>
            >>> # Remove last message (thread gets deleted)
            >>> was_deleted = thread.remove_message(last_message)
            >>> print(was_deleted)  # True - entire thread deleted
    ...
    def delete(self):
        ...
        Example:
            >>> thread = manager.get_thread_by_id('thread123')
            >>> print(f"Deleting thread: {thread.subject}")
            >>> thread.delete()
            >>> print("Thread and all messages deleted from Gmail")

        Warning:
            This operation is permanent and cannot be undone. The thread and all
    ...
    def modify_labels(
        ...
        Example:
            >>> # Add STARRED label to entire thread
            >>> thread.modify_labels(add_label_ids=['STARRED'])
            >>>
            >>> # Remove UNREAD from all messages in thread
            >>> thread.modify_labels(remove_label_ids=['UNREAD'])
            >>>
            >>> # Add custom label and remove INBOX
            >>> thread.modify_labels(
            ...     add_label_ids=['Label_123'],
            ...     remove_label_ids=['INBOX']
            ... )
            >>>
            >>> # Mark entire thread as read and important
            >>> thread.modify_labels(
    ...
    def to_dict(self) -> dict:
        ...
        Example:
            ...
            >>> json.dumps(thread_dict)  # Can be serialized to JSON
        """
        self._ensure_messages_loaded()
        return {
            "id": self.id,
...
class Message:
    ...
    Example:
        ...
        ...     impersonate_user='user@example.com'
        ... )
        >>> # Message from full data with auto-created thread
        >>> message = manager.get_message_by_message_id('abc123')
        >>> print(f"From: {message.from_email}")
        >>> print(f"Subject: {message.subject}")
        >>> print(f"Thread has {message.thread.message_count} messages")
        >>>
        >>> # Lazy-loaded message
        >>> message = Message(message_id='msg123', manager=manager)
        >>> print(message.subject)  # Triggers lazy load
        >>>
        >>> # Delete message
        >>> message.delete()  # Permanent deletion
        >>>
        >>> # Or move to trash
        >>> message.trash()  # Recoverable
        >>> message.untrash()  # Restore
        >>>
        >>> # Access thread
        >>> if message.is_unread():
        ...     print("This message is unread")
        >>> for other_msg in message.thread.messages:
        ...     print(f"Related: {other_msg.subject}")
    """
    ...
    def __init__(
        ...
        Example:
            ...
            >>> message = Message(raw_message={'id': 'abc', 'threadId': '123', ...})
            >>>
            >>> # Lazy initialization
            >>> message = Message(message_id='abc123', manager=email_manager)
            >>> print(message.subject)  # Triggers lazy load
            >>>
            >>> # With existing thread
            >>> message = Message(raw_message=raw_data, thread=my_thread)
            >>>
            >>> # With auto-created thread
            >>> message = Message(raw_message=raw_data, manager=email_manager)
            >>> print(message.thread.id)  # Thread auto-created
        """
    ...
    def is_first_in_thread(self) -> bool:
        ...
        Example:
            >>> message = manager.get_message_by_message_id('abc123')
            >>> if message.is_first_in_thread():
            ...     print("This is the thread starter!")
            ... else:
            ...     print("This is a reply in the thread")
            >>>
            >>> # Use in processing
            >>> for message in thread.messages:
            ...     if message.is_first_in_thread():
            ...         print(f"Original message from {message.from_email}")
            ...     else:
            ...         print(f"  Reply from {message.from_email}")
    ...
    def to_dict(self) -> dict:
        ...
        Example:
            ...
            >>> json.dumps(message_dict)  # Can be serialized to JSON
        """
        return {
            "id": self.id,
    ...
    def delete(self):
        ...
        Example:
            >>> message = manager.get_message_by_message_id('msg123')
            >>> print(f"Deleting message: {message.subject}")
            >>> message.delete()
            >>> print("Message permanently deleted")

        Warning:
            This operation is permanent and cannot be undone. The message will be
    ...
    def untrash(self):
        ...
        Example:
            ...
            >>> # Restore from trash
            >>> message.untrash()
            >>> print("Message restored from trash")
    ...
    def modify(self, label_ids: list, action: str = "add"):
        """
        ...
        Example:
            ...
            >>>
            >>> # Add labels
            >>> message.modify(['STARRED', 'IMPORTANT'], action='add')
            >>> print(f"Labels: {message.label_ids}")
            >>>
            >>> # Remove labels
            >>> message.modify(['UNREAD'], action='remove')
            >>> print(f"Message marked as read")
            >>>
            >>> # Add custom labels
            >>> message.modify(['Label_456', 'Label_789'], action='add')
            >>>
            >>> # Method chaining
            >>> message.modify(['INBOX'], action='add').modify(['SPAM'], action='remove')
...
class Label:
    ...
    def create_label(self):
        ...
        Example:
            ...
            ...     manager=manager
            ... )
            >>> # Label is created automatically in __init__
            >>> print(f"Created label: {label.id}")
            >>>
            >>> # Or create manually without auto-creation
            >>> label = Label(name='Manual Label', manager=manager)
            >>> label.create_label()
        """
        if not self.name:
    ...
    def get(self):
        ...
        Example:
            >>> label = Label(label_id='Label_123', manager=manager)
            >>> label.get()
            >>> print(f"Label: {label.name}")
            >>> print(f"Messages: {label.messagesTotal} ({label.messagesUnread} unread)")
        """
        if not self._manager:
            raise Exception(
    ...
    def list(manager):
        ...
        Example:
            >>> # List all labels
            >>> labels = Label.list(manager)
            >>> for label in labels:
            ...     print(f"{label.name} (ID: {label.id})")
            ...     print(f"  Type: {label.type}")
            ...     print(f"  Messages: {label.messagesTotal}")
            >>>
            >>> # Filter user labels
    ...
    def update(self):
        ...
        Example:
            ...
            >>> label.color['backgroundColor'] = '#00ff00'
            >>> label.update()
    ...
    def patch(self, **kwargs):
        ...
        Example:
            ...
            ... )
            >>>
            >>> # Patch with full color object
            >>> label.patch(
            ...     color={'textColor': '#000000', 'backgroundColor': '#ffff00'}
            ... )
        """
        if not self._manager:
            raise Exception(
    ...
    def delete(self):
        ...
        Example:
            >>> label = Label(label_id='Label_123', manager=manager)
            >>> label.delete()
            >>> print("Label deleted from Gmail")
...
class SendAsAlias:
    ...
    Example:
        ...
        >>> # Create new alias
        >>> alias = SendAsAlias(
        ...     send_as_email='support@company.com',
        ...
        >>>
        >>> # Update alias
        >>> alias.signature = '<p>Best regards,<br>Support Team</p>'
        >>> alias.update()
        >>>
        >>> # List all aliases
        >>> aliases = manager.list_aliases()
        >>> for alias in aliases:
        ...     print(f"{alias.display_name} <{alias.send_as_email}>")
    """
    ...
    def create(self):
        ...
        Example:
            ...
            ...     signature='<p>Best regards,<br>Support</p>',
            ...     manager=manager
            ... )
            >>> alias.create()
            >>> print(f"Created alias: {alias.send_as_email}")
            >>> print(f"Verification status: {alias.verification_status}")
        """
        if not self._manager:
            raise Exception(
    ...
    def get(self):
        ...
        Example:
            >>> alias = SendAsAlias(send_as_email='support@company.com', manager=manager)
            >>> alias.get()
            >>> print(f"Display name: {alias.display_name}")
            >>> print(f"Verified: {alias.is_verified}")
        """
        if not self._manager:
            raise Exception(
    ...
    def update(self):
        ...
        Example:
            >>> alias = SendAsAlias(send_as_email='support@company.com', manager=manager)
            >>> alias.get()
            >>> alias.display_name = 'Customer Support'
            >>> alias.signature = '<p>Thanks,<br>Support Team</p>'
            >>> alias.reply_to_address = 'noreply@company.com'
            >>> alias.update()
    ...
    def patch(self, **kwargs):
        ...
        Example:
            >>> alias = SendAsAlias(send_as_email='support@company.com', manager=manager)
            >>> # Only update signature
            >>> alias.patch(signature='<p>New signature</p>')
            >>>
            >>> # Update multiple fields
            >>> alias.patch(
            ...     display_name='New Name',
            ...     reply_to_address='replies@company.com'
            ... )
        """
        if not self._manager:
            raise Exception(
    ...
    def delete(self):
        ...
        Example:
            >>> alias = SendAsAlias(send_as_email='old@company.com', manager=manager)
            >>> alias.delete()
    ...
    def verify(self):
        ...
        Example:
            >>> alias = SendAsAlias(
            ...     send_as_email='newaddress@company.com',
            ...     manager=manager
            ... )
            >>> alias.create()
            ...
            >>> else:
            ...     # Can still resend verification if needed
            ...     alias.verify()
...
class UserAlias:
    ...
    Example:
        ...
        >>> aliases = UserAlias.list(manager, 'john.doe@company.com')
        >>> for alias in aliases:
    ...
    def __init__(
        ...
        Example:
            ...
            >>>
            >>> # From API response
            >>> alias = UserAlias(raw_alias={'alias': 'jdoe@company.com', ...}, manager=manager)
        """
        self._manager = manager
    ...
    def create(self):
        ...
        Example:
            ...
            ... )
            >>> alias.create()
            >>> print(f"Created alias: {alias.alias}")
    ...
    def delete(self):
        ...
        Example:
            >>> alias = UserAlias(
            ...     alias='old-alias@company.com',
            ...     primary_email='john.doe@company.com',
            ...     manager=manager
            ... )
            >>> alias.delete()
            >>> print(f"Deleted alias: {alias.alias}")
    ...
    def to_dict(self) -> dict:
        ...
        Example:
            >>> alias_dict = alias.to_dict()
            >>> json.dumps(alias_dict)  # Can be serialized to JSON
        """
        return {
            "alias": self.alias,
...
class ServiceAccountEmailManager:
    ...
    Example:
        ...
        ...     file_attachment_paths=['welcome.pdf']
        ... )
        >>>
        >>> # Read an email
        >>> message = manager.get_message_by_message_id('abc123')
        ...
        >>> message = manager.get_message_by_message_id_for_user(
        ...     '',
        ...     'xyz789'
        ... )
        >>>
        ...
        >>>
        >>> # List threads
        >>> result = manager.list_threads(max_results=20, label_ids=['INBOX'])
        >>> for thread in result['threads']:
        ...     print(f"{thread.subject} ({thread.message_count} msgs)")
    ...
    def __init__(
        ...
        Example:
            ...
            >>> manager = ServiceAccountEmailManager()
            >>>
            >>> # Specify service account and user
            >>> manager = ServiceAccountEmailManager(
            ...     service_account_file='cred/noreply_sa.json',
            ...     impersonate_user='no-reply@example.com'
            ... )
            >>>
            >>> # Add custom scopes
            >>> manager = ServiceAccountEmailManager(
    ...
    def send_email(
        ...
        from_name = from_name or config.get("from_name", "Meno Enterprises")
    ...
    def get_message_by_message_id(self, message_id: str, format: str = "full"):
        ...
        Example:
            email = manager.get_email_by_message_id('18c5a1b2f3d4e5f6')
            print(f"From: {email.from_email}")
            print(f"Subject: {email.subject}")
            print(f"Body: {email.body}")

            # Access raw headers if needed
            print(f"Message-ID: {email.headers.get('Message-ID')}")

            # Access thread
            print(f"Thread subject: {email.thread.subject}")
            print(f"Thread has {email.thread.message_count} messages")
        """
        try:
    ...
    def get_message_by_message_id_for_user(
        ...
        Example:
            ...
            print(f"Accessed as: {email1.accessed_as_user}")
            print(f"From: {email1.from_email}")
    ...
    def get_messages_from_pubsub(self, pubsub_data):
        ...
        IMPORTANT: The notification does NOT contain Gmail message IDs! You must:
    ...
    def get_account_info(self):
        ...
        Example:
            ...
            >>> print(f"Total messages: {info['messages_total']}")
            >>> print(f"Total threads: {info['threads_total']}")
    ...
    def watch(
        ...
        Args:
            topic_name (str, optional): Full Pub/Sub topic name in the format
                ...
                PTS_CSR_TOPIC_NAME from environment variables.
        ...
        Raises:
            ValueError: If topic_name is not provided and PTS_CSR_TOPIC_NAME env var is not set
            Exception: If the API call fails
        ...
        Example:
            ...
            ... )
            >>>
            >>> # Watch all mailbox changes (uses PTS_CSR_TOPIC_NAME from .env)
        ...
        if topic_name is None:
            topic_name = os.getenv("PTS_CSR_TOPIC_NAME")
            if not topic_name:
                raise ValueError(
                    "topic_name parameter not provided and PTS_CSR_TOPIC_NAME "
...
if __name__ == "__main__":
    ...
    #     manager = ServiceAccountEmailManager(
    #         service_account_file='cred/noreply_sa.json',
    #         impersonate_user='no-reply@example.com'  # Default user
    #     )
    #
    #     # Get email from different users
    #     email1 = manager.get_email_by_message_id_for_user(
